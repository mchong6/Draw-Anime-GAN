...
class _netG_1(nn.Module):
    def __init__(self, ngpu, nz, nc , ngf, n_extra_layers_g, pix_shuf):
        ...
        #self.ngf = ngf
        #if pix_shuf:
        #    self.conv1 = self.upsample_2x(nz, ngf*8*4, 4, 1, 0)
        #    self.conv2 = self.upsample_2x(ngf*8, ngf*4*4, 4, 2, 1)
        #    self.conv3 = self.upsample_2x(ngf*4, ngf*2*4, 4, 2, 1)
        #    self.conv4 = self.upsample_2x(ngf*2, ngf*4, 4, 2, 1)
        #    self.conv_out = self.upsample_2x(ngf, nc*4, 4, 2, 1)
        #else:
        ...
        layers = []
        if pix_shuf:
            layers += self.upsample_2x(nz, ngf*8*4, 4, 1, 0)
            layers += self.upsample_2x(ngf*8, ngf*4*4, 4, 2, 1)
            layers += self.upsample_2x(ngf*4, ngf*2*4, 4, 2, 1)
            layers += self.upsample_2x(ngf*2, ngf*4, 4, 2, 1)
    ...
    def upsample_2x(self, inch, outch, ksize, stride, pad):
        layers = [nn.ConvTranspose2d(inch, outch, ksize, stride, pad, bias=False), 
                nn.PixelShuffle(2),
                nn.BatchNorm2d(outch / 4), # PixelShuffle reduces channel to a quarter
                nn.LeakyReLU(0.2, inplace=True)]
        return layers

    def deconvolution(self, inch, outch, ksize, stride, pad):
        layers = [nn.ConvTranspose2d(inch, outch, ksize, stride, pad, bias=False), 
                nn.PixelShuffle(2),
                nn.BatchNorm2d(outch / 4), # PixelShuffle reduces channel to a quarter
                nn.LeakyReLU(0.2, inplace=True)]
